...
- Name: e2e/topology-interaction.spec.js >> Topology Interactions >> clicking a connection line opens InfoPanel
- Location: tests/e2e/topology-interaction.spec.js:77:7
...
Locator: locator('text="CONNECTION-LINE"')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text="CONNECTION-LINE"')
...
- button "+"
- button "APPLY"
- text: "ELEMENTS: 227 CONNECTION-LINE: conn-bridge-0-0-0-line-0"
- button "+"
- button "−"
- button "⊡"
- button "↺"
- text: "ZOOM: 74% FROM → ← TO ↔ BOTH connection-line"
- button "Close": ×
- text: id conn-bridge-0-0-0-line-0 connectionId conn-bridge-0-0-0 from router-0-0 to bridge-0-0-0 direction ↔ BIDIRECTIONAL lineIndex 1 / 1 offset 0.0
- alert
```
...
```ts
  3   | import { simulateClick, simulateHover, getElementById, queryElements } from '../helpers/mouse-simulation.js';
  4   | 
  5   | test.describe('Topology Interactions', () => {
  6   |   test.beforeEach(async ({ page }) => {
  7   |     await navigateAndWait(page);
  8   |   });
  9   | 
  10  |   test('hovering an element updates the HUD', async ({ page }) => {
