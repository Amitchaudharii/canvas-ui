...
- Name: e2e/topology-viewport.spec.js >> Topology Viewport >> reset button restores view
- Location: tests/e2e/topology-viewport.spec.js:58:7

# Error details

```
Error: locator.click: Error: strict mode violation: locator('button').filter({ hasText: '+' }) resolved to 3 elements:
    1) <button>+</button> aka getByRole('button', { name: '+' }).first()
    2) <button>+</button> aka getByRole('button', { name: '+' }).nth(1)
    3) <button title="Zoom In">+</button> aka getByRole('button', { name: '+', description: 'Zoom In', exact: true })
