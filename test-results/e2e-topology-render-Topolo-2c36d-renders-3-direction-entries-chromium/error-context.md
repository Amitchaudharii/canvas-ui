...
- Name: e2e/topology-render.spec.js >> Topology Rendering >> ArrowLegend renders 3 direction entries
- Location: tests/e2e/topology-render.spec.js:31:7
...
Locator: locator('div').filter({ hasText: 'FROM →' }).locator('..')
Expected: visible
Error: strict mode violation: locator('div').filter({ hasText: 'FROM →' }).locator('..') resolved to 4 elements:
    1) <body>…</body> aka locator('body')
    2) <div>…</div> aka getByText('GRIDROWS−+×COLS−+APPLYELEMENTS: 227HOVER OR CLICK TO SELECT+−⊡↺ZOOM: 74%FROM')
    3) <div>…</div> aka getByText('ELEMENTS: 227HOVER OR CLICK TO SELECT+−⊡↺ZOOM: 74%FROM →← TO↔ BOTH')
    4) <div>…</div> aka getByText('FROM →← TO↔ BOTH')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('div').filter({ hasText: 'FROM →' }).locator('..')
