...
- Name: e2e/topology-render.spec.js >> Topology Rendering >> HUD renders correct element count
- Location: tests/e2e/topology-render.spec.js:20:7
...
Locator: locator('div').filter({ hasText: /^ELEMENTS:/ })
Expected: visible
Error: strict mode violation: locator('div').filter({ hasText: /^ELEMENTS:/ }) resolved to 3 elements:
    1) <div>…</div> aka getByText('ELEMENTS: 227HOVER OR CLICK TO SELECT+−⊡↺ZOOM: 74%FROM →← TO↔ BOTH')
    2) <div>…</div> aka getByText('ELEMENTS: 227HOVER OR CLICK')
    3) <div>…</div> aka getByText('ELEMENTS:')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('div').filter({ hasText: /^ELEMENTS:/ })
